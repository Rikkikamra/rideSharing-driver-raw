...
import { submitTripFeedback } from '../utils/api';
...
const TripFeedbackScreen = ({ navigation, route }) => {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    const result = await submitTripFeedback(route.params?.tripId, feedback);
    setLoading(false);
    if (result?.success) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <Text style={[styles.header, { color: colors.primary }]}>Trip Feedback</Text>
      <Text style={{ color: colors.text, marginBottom: 10 }}>
        How was your recent trip? Share your feedback below.
      </Text>
      <TextInput
